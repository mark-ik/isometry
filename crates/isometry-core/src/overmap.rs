...
mod tests {
    ...
    fn route_takes_the_cheaper_path_by_weight() {
        ...
        assert_eq!(
            path,
            vec!["village", "forest", "ruins"],
            "the cheaper of two ways"
        );
        assert_eq!(cost, 5);
    }
    ...
    fn reachable_within_is_bounded_by_the_travel_budget() {
        ...
        assert_eq!(
            far.get("ruins"),
            Some(&5),
            "the cheap forest route, not the swamp's 6"
        );
    ...
    fn a_directed_route_is_one_way() {
        ...
        assert!(
            m.route("top", "base").is_some(),
            "you can descend the cliff"
        );
        assert!(m.route("base", "top").is_none(), "but not climb back up it");
        assert_eq!(m.neighbours("top"), vec![("base", 1)]);
        assert!(
            m.neighbours("base").is_empty(),
            "a directed edge is not a neighbour of its target"
        );
    }
    ...
    fn an_unreachable_or_unknown_site_has_no_route() {
        ...
        assert!(
            m.route("village", "island").is_none(),
            "no route to the island"
        );
