...
| [2026-07-14_adjudication_and_representation_plan.md](2026-07-14_adjudication_and_representation_plan.md) | Active plan, the game lane: **the app adjudicates** (answers next-horizons B.4 and releases its fork-gated lane 4). Resolve once, replicate the outcome, represent locally: `ActionIntent` → `ActionResolved { roll, outcome, deltas, beats }`, applied by peers who never rerun Lua. **Beats** are one primitive for combat, environment, and emotes, carried by genet's CSS animation clock. A0 landed (clock driven from the host; worst-case beat 10.1 ms / ~72 fps, idle costs one frame; Isometry is genet's first production consumer of the animation lane). A1 beat element, A2 the targeted action loop, A3-A5 beats/emotes/pack choreography. |
